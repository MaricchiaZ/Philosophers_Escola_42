...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/04/13 10:45:05 by maclara-          #+#    #+#             */
/*   Updated: 2023/04/23 13:20:10 by maclara-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_args(int argc, char **argv)
{
	...
	if (argc < 5 || argc > 6)  // só podemos ter 5 ou 6 argumentos, se for diferente disso
	{
		...
		ft_putstr_fd("Include: nbr_philo time_to_starve \
		time_eating time_sleeping (nbr_of_meals - opitional).\n", 2); // e orientamos como passar os argumentos
	...
	while (argv[i])  // vamos olhar argumento por argumento, para ver se são numéricos, positivos e não estouram o INT_MAX
	{
		...
		{
			ft_putstr_fd("Invalid arguments! Arguments \
			must be numbers integers and positive.\n", 2); // avisamos o erro
			return (0); // retornamos falso ou erro
		}
